...
<div class="space-y-4 sm:space-y-6">
    ...
    <div class="bg-white rounded-2xl border p-4 sm:p-6">
        <form action="{{ route('cms.products.update', $component->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="domain_id" id="domain_id" class="w-full px-4 py-2 rounded-lg border-2 border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all">
                    ...
                    @foreach($domains as $domain)
                        <option value="{{ $domain->id }}" {{ (old('domain_id', $component->domain_id) == $domain->id) ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $component->properties['title'] ?? '') }}" class="w-full px-4 py-2 rounded-lg border-2 border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all" placeholder="Masukkan nama produk">
                @error('title')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" class="w-full px-4 py-2 rounded-lg border-2 border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all" placeholder="Masukkan deskripsi produk">{{ old('description', $component->properties['description'] ?? '') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" name="price" id="price" value="{{ old('price', $component->properties['price'] ?? 0) }}" min="0" step="1000" class="w-full px-4 py-2 rounded-lg border-2 border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all" placeholder="0">
                @error('price')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Gambar Produk Saat Ini</label>
                <div class="mb-3" id="currentImage">
                    <img src="{{ $component->properties['image'] ?? 'https://placehold.co/400x300' }}" alt="Current product image" class="w-32 h-32 object-cover rounded-lg border-2 border-gray-300">
                </div>
                ...
                <label for="image" class="block text-sm font-medium text-gray-700 mb-1">Ganti Gambar Produk</label>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">File Produk Digital Saat Ini</label>
                <div class="mb-3" id="currentFile">
                    @if(!empty($component->properties['digitalProduct']['originalName']))
                        <div class="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
                            <svg class="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                            </svg>
                            <span class="text-sm">{{ $component->properties['digitalProduct']['originalName'] }}</span>
                        </div>
                    @else
                        <p class="text-sm text-gray-500">Tidak ada file digital</p>
                    @endif
                </div>
                ...
                <label for="digital_product" class="block text-sm font-medium text-gray-700 mb-1">Ganti File Produk Digital</label>
